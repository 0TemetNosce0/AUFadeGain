...
void EngineXfader::getXfadeGains(double xfadePosition, double transform, double powerCalibration, double curve,
                                 bool reverse, double *gain1)
{
    double gain2 = 0;
    if (gain1 == nullptr) {
        return;
    ...
    if (xfadePositionRight > 0) { // right side
        *gain1 = (1.0 - (1.0 * std::pow(xfadePositionRight, transform)));
    } else {
        *gain1 = 1.0;
    }
    ...
    if (curve == MIXXX_XFADER_CONSTPWR) {
        ...
        double gain = std::sqrt(*gain1 * *gain1 + gain2 * gain2);
        *gain1 = *gain1 / gain;
        gain2 = gain2 / gain;
    }
